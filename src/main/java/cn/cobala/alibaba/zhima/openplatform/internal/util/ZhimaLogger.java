...
public class ZhimaLogger {
    ...
    public static void setNeedEnableLogger(boolean needEnableLogger) {
        ZhimaLogger.needEnableLogger = needEnableLogger;
    }
    ...
    private static void _logCommError(Exception e, HttpURLConnection conn, String url,
        ...
        String sdkName = ZhimaConstants.SDK_VERSION;
        String urlStr = null;
        String rspCode = "";
        if (conn != null) {
            try {
     ...
     * 发生特别错误时记录完整错误现场
     */
    public static void logErrorScene(Map<String, Object> rt, ZhimaResponse tRsp, String appSecret) {
        if (!needEnableLogger) {
            return;
        }
        DateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        ...
        sb.append(df.format(new Date()));
        sb.append("^_^");
        sb.append("ProtocalParams:");
        appendLog((ZhimaHashMap) rt.get("protocalParams"), sb);
        sb.append("^_^");
        sb.append("ApplicationParams:");
        appendLog((ZhimaHashMap) rt.get("textParams"), sb);
        sb.append("^_^");
        sb.append("Body:");
    ...
    private static void appendLog(ZhimaHashMap map, StringBuilder sb) {
        boolean first = true;
